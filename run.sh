#!/usr/bin/env bash
# SFT cold-start 4B (Direction 2 at 4B): LoRA SFT of Qwen3-4B on the SAME teacher demos
# (alphaXiv/chroma-sft-traces), export merged HF model, push adapter to HF for the RL node.
# Identical to the 1.7B SFT node except MODEL=Qwen/Qwen3-4B. Command: bash run.sh.
set -euxo pipefail
cd "$(dirname "$0")"
ROOT="$PWD"
...
api.upload_folder(folder_path=src, repo_id=repo, commit_message="SFT cold-start 4B merged model")
print(repo)
PY
)
fi
...
# ---------- EVAL.md ----------
{
  echo "# SFT cold-start 4B — $RUN_NAME (exit $SFT_EXIT)"
  echo
  echo "## Export"
