#!/usr/bin/env bash
# RL-from-SFT 4B (Direction 2 at 4B): identical to Stage 4c CISPO RL (4B snippet, budget
# 4096, micro_forward=1), but the policy is initialized from the 4B SFT cold-start model.
# Preamble merges the SFT LoRA adapter (alphaXiv/chroma-sft-qwen3-4b) into base Qwen3-4B.
# eval_before_train=true => step-0 eval = SFT-only 4B. Command: bash run.sh.
set -euxo pipefail
cd "$(dirname "$0")"
ROOT="$PWD"

BASE_MODEL="Qwen/Qwen3-4B"
SFT_ADAPTER="alphaXiv/chroma-sft-qwen3-4b"
MERGED="$HOME/sft_merged"
MODEL="$MERGED"
RUN_NAME="stage-sft-rl-qwen3-4b-snippet"
...
# ---------- dataset (same deterministic build as 4c; seed 42) ----------
.venv/bin/python chroma/build_dataset.py --out data --n-train 1000 --n-val 200 --pool-extra 990 --eval-md data_EVAL.md

# ---------- training (identical hyperparams to Stage 4c 4B; only the model path differs) ----------
set +e
...
# ---------- EVAL.md ----------
{
  echo "# RL-from-SFT 4B snippet search (Qwen3-4B) — $RUN_NAME (exit $TRAIN_EXIT)"
  echo
  echo "## Policy init"
